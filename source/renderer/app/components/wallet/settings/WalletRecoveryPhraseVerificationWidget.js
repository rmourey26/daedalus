...
export default class WalletRecoveryPhraseVerificationWidget extends Component<Props> {
  ...
  get recoveryPhraseStatus() {
    ...
    const timeAgo = dayjs(recoveryPhraseVerificationDate).fromNow();
    const timeFromCreationToWarning = dayjs(new Date(creationDate)).add(
      times.warning,
      'days'
    );
    let timeUntilWarning = dayjs()
      .locale(locale)
      .to(timeFromCreationToWarning, true);

    // The content is generated by `dayjs`, but we need to replace `ヶ月` by `か月`
